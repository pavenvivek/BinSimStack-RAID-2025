...
def generate_function_data(proj, cfg, func, trs, filename):
    ...
    for i in blk_addrs:
        ...
        #for n in nodes:
        #    if i == n.addr:
        #        if type(n) == angr.codenode.BlockNode and n._graph is not None:
        #            succs = n.successors()
        #        break
                
        #for s in succs:
        #    if type(s) == angr.codenode.BlockNode:
        #        graph[i].append(s.addr)
        #    #elif type(s) == angr.knowledge_plugins.functions.function.Function:
        #    #    graph[i].append(s.name)
                ...
                try:
                    ...
                    for j in cfg.kb.functions:
                        if str(cfg.kb.functions[j].name) in fn_trs_lst[os.path.basename(fl)]:
                            f_info = generate_function_data(proj, cfg, cfg.kb.functions[j], trs, os.path.basename(fl)) # N/A -> no transformation applied
