...
# Siamese triplet implementation
class graphnn(object):
    def __init__(self,
        ...
        with tf.device(device):
            ...
            X3 = tf.compat.v1.placeholder(Dtype, [None, None, N_x]) #[B, N_node, N_x]
            msg3_mask = tf.compat.v1.placeholder(Dtype, [None, None, None])
                                            #[B, N_node, N_node]
            self.X3 = X3
            self.msg3_mask = msg3_mask
            embed3 = graph_embed(X3, msg3_mask, N_x, N_embed, N_o, ITER_LEVEL,
                    Wnode, Wembed, W_output, b_output)  #[B, N_x]
            ...
            # uncomment this for Gemini (Siamese twin  network and MSE loss function) 
            ...
            pos = label[0] * (diff[0] - diff1[0]) + 0.2            
            neg = label[0] * (diff1[1] - diff[1]) + 0.2

            # Triplet loss function for Siamese triplet network
            loss = tf.maximum(pos, 0) + tf.maximum(neg, 0)
            self.loss = loss
    ...
    def calc_loss(self, X1, X2, X3, mask1, mask2, mask3, y):
        cur_loss, = self.sess.run(fetches=[self.loss], feed_dict={self.X1:X1,
            self.X2:X2,self.X3:X3,self.msg1_mask:mask1,self.msg2_mask:mask2,self.msg3_mask:mask3,self.label:y})
        return cur_loss
        
    def calc_diff(self, X1, X2, X3, mask1, mask2, mask3):
    ...
    def train(self, X1, X2, X3, mask1, mask2, mask3, y):
