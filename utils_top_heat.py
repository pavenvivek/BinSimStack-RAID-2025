...
def get_auc_epoch_combined_mtx(data_lst, mode=None):
    ...
    for i, func, diff, diff1, diff3, diff4, diff5, diff6, diff7, diff8, y, y2, y3, y4 in data_lst: 
        ...
        for j in range(0, hf):
            ...
            #if ((1 * -1 * diff7[j]) + (0 * -1 * diff5[j]) + (0 * -1 * diff3[j]) + (0 * -1 * diff[j])) > ((1 * -1 * diff8[j]) + (0 * -1 * diff6[j]) + (0 * -1 * diff4[j]) + (0 * -1 * diff1[j])):
            #if ((1 * -1 * diff7[j]) + (0 * -1 * diff5[j]) + (0 * -1 * diff3[j]) + (0 * -1 * diff[j])) > ((1 * -1 * diff8[j]) + (0 * -1 * diff6[j]) + (0 * -1 * diff4[j]) + (0 * -1 * diff1[j])):
        ...
        top_dict[func]["top"] = false_pred_cnt_lcl + 1
    ...
    for k,v in top_dict.items():
        print (f"{k} --> index: {top_dict[k]['index']}, value: {top_dict[k]['top']}")
