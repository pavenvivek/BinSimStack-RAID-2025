...
def get_auc_epoch_combined_mtx(data_lst, mode=None):
    ...
    fnc_cnt = 0
    for i, func, diff, diff1, diff3, diff4, diff5, diff6, diff7, diff8, y, y2, y3, y4 in data_lst: 
        ...
        for j in range(0, hf):
            ...
            if (-1 * diff7[j]) > (-1 * diff8[j]):
                acc_4 = acc_4 + 1
                pred4 = True
            else:
                mdl4_fls_pred_cnt = mdl4_fls_pred_cnt + 1
            ...
            acc_cnt_4 = acc_cnt_4 + 1


            dif_val_1 = abs((-1 * diff[j]) - (-1 * diff1[j]))
            dif_val_2 = abs((-1 * diff3[j]) - (-1 * diff4[j]))
            dif_val_3 = abs((-1 * diff5[j]) - (-1 * diff6[j]))
            dif_val_4 = abs((-1 * diff7[j]) - (-1 * diff8[j]))
                
            if dif_val_1 == max(dif_val_1, dif_val_2, dif_val_3, dif_val_4):
                if (-1 * diff[j]) > (-1 * diff1[j]):
                    mdl_best_pred = mdl_best_pred + 1
            elif dif_val_2 == max(dif_val_1, dif_val_2, dif_val_3, dif_val_4):
                if (-1 * diff3[j]) > (-1 * diff4[j]):
                    mdl_best_pred = mdl_best_pred + 1
            elif dif_val_3 == max(dif_val_1, dif_val_2, dif_val_3, dif_val_4):
                if (-1 * diff5[j]) > (-1 * diff6[j]):
                    mdl_best_pred = mdl_best_pred + 1
            elif dif_val_4 == max(dif_val_1, dif_val_2, dif_val_3, dif_val_4):
                if (-1 * diff7[j]) > (-1 * diff8[j]):
                    mdl_best_pred = mdl_best_pred + 1

            # model weights for flt (using top-1 from scale 500)
            #w1 = 0.50    
            #w2 = 0.80    
            #w3 = 0.78    

            # model weights for viz (from scale 500)
            #w1 = 0.24
            #w2 = 0.45
            #w3 = 0.45

            # model weights for O0 <-> O1 (from scale 500)
            #w1 = 0.60
            #w2 = 0.63
            #w3 = 0.33
            ...
            # model weights for O0 <-> O2 (from binutils)
            w1 = 0.28
            w2 = 0.29
            w3 = 0.14

            # model weights for O0 <-> O3 (from binutils)
            w1 = 0.26
            w2 = 0.20
            w3 = 0.16
            ...
            if (w1 * dif_val_1) == max((w1 * dif_val_1), (w2 * dif_val_2), (w3 * dif_val_3), (w4 * dif_val_4)):
                if (-1 * diff[j]) > (-1 * diff1[j]):
                    mdl_wt_best_pred = mdl_wt_best_pred + 1
            elif (w2 * dif_val_2) == max((w1 * dif_val_1), (w2 * dif_val_2), (w3 * dif_val_3), (w4 * dif_val_4)):
                if (-1 * diff3[j]) > (-1 * diff4[j]):
                    mdl_wt_best_pred = mdl_wt_best_pred + 1
            elif (w3 * dif_val_3) == max((w1 * dif_val_1), (w2 * dif_val_2), (w3 * dif_val_3), (w4 * dif_val_4)):
                if (-1 * diff5[j]) > (-1 * diff6[j]):
                    mdl_wt_best_pred = mdl_wt_best_pred + 1
            elif (w4 * dif_val_4) == max((w1 * dif_val_1), (w2 * dif_val_2), (w3 * dif_val_3), (w4 * dif_val_4)):
                if (-1 * diff7[j]) > (-1 * diff8[j]):
                    mdl_wt_best_pred = mdl_wt_best_pred + 1

                    
            if pred1 == True or pred2 == True or pred3 == True or pred4 == True:
                mdl_or_pred = mdl_or_pred + 1

            if pred1 == True and pred2 == True and pred3 == True and pred4 == True:
                mdl_and_pred = mdl_and_pred + 1

            lst = [pred1, pred2, pred3, pred4]
            if lst.count(True) > 2:
            ...
            # weighted aggregate
            if ((w4 * -1 * diff7[j]) + (w3 * -1 * diff5[j]) + (w2 * -1 * diff3[j]) + (w1 * -1 * diff[j])) > ((w4 * -1 * diff8[j]) + (w3 * -1 * diff6[j]) + (w2 * -1 * diff4[j]) + (w1 * -1 * diff1[j])): #(top-1 weights)
            ...
            # no weights    
            #if ((w4 * -1 * diff7[j]) + (0 * -1 * diff5[j]) + (w2 * -1 * diff3[j]) + (w1 * -1 * diff[j])) > ((w4 * -1 * diff8[j]) + (0 * -1 * diff6[j]) + (w2 * -1 * diff4[j]) + (w1 * -1 * diff1[j])): #(top-1 weights)
            if ((w4 * -1 * diff7[j]) + (w3 * -1 * diff5[j]) + (w2 * -1 * diff3[j]) + (w1 * -1 * diff[j])) > ((w4 * -1 * diff8[j]) + (w3 * -1 * diff6[j]) + (w2 * -1 * diff4[j]) + (w1 * -1 * diff1[j])): #(top-1 weights)
            #if ((0 * -1 * diff5[j]) + (0 * -1 * diff3[j]) + (0 * -1 * diff[j])) > ((1 * -1 * diff6[j]) + (0 * -1 * diff4[j]) + (0 * -1 * diff1[j])):
            #if ((0 * -1 * diff7[j]) + (0 * -1 * diff5[j]) + (0 * -1 * diff3[j]) + (1 * -1 * diff[j])) > ((0 * -1 * diff8[j]) + (0 * -1 * diff6[j]) + (0 * -1 * diff4[j]) + (1 * -1 * diff1[j])):
            #if ((0 * -1 * diff7[j]) + (0 * -1 * diff5[j]) + (1 * -1 * diff3[j]) + (0 * -1 * diff[j])) > ((0 * -1 * diff8[j]) + (0 * -1 * diff6[j]) + (1 * -1 * diff4[j]) + (0 * -1 * diff1[j])):
            #if ((0 * -1 * diff7[j]) + (1 * -1 * diff5[j]) + (0 * -1 * diff3[j]) + (0 * -1 * diff[j])) > ((0 * -1 * diff8[j]) + (1 * -1 * diff6[j]) + (0 * -1 * diff4[j]) + (0 * -1 * diff1[j])):
            #if ((1 * -1 * diff7[j]) + (1 * -1 * diff5[j]) + (1 * -1 * diff3[j]) + (0 * -1 * diff[j])) > ((1 * -1 * diff8[j]) + (1 * -1 * diff6[j]) + (1 * -1 * diff4[j]) + (0 * -1 * diff1[j])):
            #if ((1 * -1 * diff7[j]) + (1 * -1 * diff5[j]) + (1 * -1 * diff3[j]) + (1 * -1 * diff[j])) > ((1 * -1 * diff8[j]) + (1 * -1 * diff6[j]) + (1 * -1 * diff4[j]) + (1 * -1 * diff1[j])):
                acc_cmb_2 = acc_cmb_2 + 1
            else:
        ...
        MRR_sum = MRR_sum + (1/(false_pred_cnt_lcl + 1))    
        ...
        if mdl4_fls_pred_cnt == 0:
            mdl4_pred_top_1.append(i)
            mdl4_pred_top_2.append(i)
            mdl4_pred_top_3.append(i)
            mdl4_pred_top_4.append(i)
            mdl4_pred_top_5.append(i)
        elif mdl4_fls_pred_cnt == 1:
            mdl4_pred_top_2.append(i)
            mdl4_pred_top_3.append(i)
            mdl4_pred_top_4.append(i)
            mdl4_pred_top_5.append(i)
        elif mdl4_fls_pred_cnt == 2:
            mdl4_pred_top_3.append(i)
            mdl4_pred_top_4.append(i)
            mdl4_pred_top_5.append(i)
        elif mdl4_fls_pred_cnt == 3:
            mdl4_pred_top_4.append(i)
            mdl4_pred_top_5.append(i)
        elif mdl4_fls_pred_cnt == 4:
            mdl4_pred_top_5.append(i)
    ...
    print ("jTrans  accuracy: {}".format((acc_4/acc_cnt_2) * 100))
    ...
    print (f"\ntop_1_test -> {(top_1_test/fnc_cnt) * 100}, top_1_test -> {top_1_test}\n")
    ...
    print (f"\n### MRR -> {(MRR_sum/fnc_cnt) * 100}\n")

    

    #print (f"Cirrina: {mdl1_uniq_aft_agg_top_1}")
    #print (f"Gem_trp: {mdl2_uniq_aft_agg_top_1}")
    #print (f"Palmtree: {mdl3_uniq_aft_agg_top_1}")
    #print (f"Aggregate: {agg_uniq_top_1}")
    #print (f"Mdl cmb: {total_top_1_cmb}")

    print_stat(1, mdl1_pred_top_1, mdl2_pred_top_1, mdl3_pred_top_1, mdl4_pred_top_1, agg_pred_top_1, wt_agg_pred_top_1,
    ...
    print_stat(2, mdl1_pred_top_2, mdl2_pred_top_2, mdl3_pred_top_2, mdl4_pred_top_2, agg_pred_top_2, wt_agg_pred_top_2,
    ...
    print_stat(3, mdl1_pred_top_3, mdl2_pred_top_3, mdl3_pred_top_3, mdl4_pred_top_3, agg_pred_top_3, wt_agg_pred_top_3,
    ...
    print_stat(4, mdl1_pred_top_4, mdl2_pred_top_4, mdl3_pred_top_4, mdl4_pred_top_4, agg_pred_top_4, wt_agg_pred_top_4,
    ...
    print_stat(5, mdl1_pred_top_5, mdl2_pred_top_5, mdl3_pred_top_5, mdl4_pred_top_5, agg_pred_top_5, wt_agg_pred_top_5,
    ...
    return model_auc, fpr, tpr, thres, auc2_1, auc2_2, auc2_3, auc2_4, auc_cmb2 


def print_stat(top, mdl1_pred, mdl2_pred, mdl3_pred, mdl4_pred, agg_pred, wt_agg_pred, mdl_or_pred_avg, mdl_and_pred_avg, mdl_vote_pred_avg, mdl_best_pred_avg, mdl_wt_best_pred_avg, fnc_cnt):

    total_m2_m3_m4 = list(set(mdl2_pred).union(set(mdl3_pred).union(set(mdl4_pred))))
    total_m1_m3_m4 = list(set(mdl1_pred).union(set(mdl3_pred).union(set(mdl4_pred))))
    total_m1_m2_m4 = list(set(mdl1_pred).union(set(mdl2_pred).union(set(mdl4_pred))))
    total_m1_m2_m3 = list(set(mdl1_pred).union(set(mdl2_pred).union(set(mdl3_pred))))
    total = list(set(mdl1_pred).union(set(mdl2_pred).union(set(mdl3_pred).union(set(mdl4_pred)))))

    mdl1_uniq_bef_agg = list(set(mdl1_pred) - set(total_m2_m3_m4))
    mdl1_uniq_aft_agg = list(set(mdl1_uniq_bef_agg) - set(agg_pred))

    mdl2_uniq_bef_agg = list(set(mdl2_pred) - set(total_m1_m3_m4))
    mdl2_uniq_aft_agg = list(set(mdl2_uniq_bef_agg) - set(agg_pred))

    mdl3_uniq_bef_agg = list(set(mdl3_pred) - set(total_m1_m2_m4))
    mdl3_uniq_aft_agg = list(set(mdl3_uniq_bef_agg) - set(agg_pred))

    mdl4_uniq_bef_agg = list(set(mdl4_pred) - set(total_m1_m2_m3))
    mdl4_uniq_aft_agg = list(set(mdl4_uniq_bef_agg) - set(agg_pred))
    
    agg_missed = list(set(total) - set(agg_pred))
    agg_uniq = list(set(agg_pred) - set(total))
    agg_retained = list(set(agg_pred) - set(agg_uniq))
    #wt_agg_uniq = list(set(wt_agg_pred) - set(total))
    total_cmb = list(set(total) - set(mdl1_uniq_aft_agg).union(set(mdl2_uniq_aft_agg).union(set(mdl3_uniq_aft_agg).union(set(mdl4_uniq_aft_agg)))))
    ...
    print(f"jTrans         : total -> {(len(mdl4_pred)/fnc_cnt) * 100}, unique -> bef. aggregate: {(len(mdl4_uniq_bef_agg)/fnc_cnt) * 100}, aft. aggregate: {(len(mdl4_uniq_aft_agg)/fnc_cnt) * 100} ")
    ...
    print(f"Agg. retained  : total -> {(len(agg_retained)/fnc_cnt) * 100} ")
    #print(f"Model combined : total -> {(len(total_cmb)/fnc_cnt) * 100} ")
    print(f"Model union    : total -> {(len(total)/fnc_cnt) * 100} ")
    print (f"\nTop-{top} info:")
